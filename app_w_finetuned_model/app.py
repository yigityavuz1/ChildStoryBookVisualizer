...
lora_weights_path = "sd-model-finetuned/pytorch_lora_weights.safetensors"
# -------------------------------
...
def generate_image(prompt_text, pipe):
    """Generates an image based on the prompt using the Hugging Face Inference API."""
    image_data = pipe(prompt_text, num_inference_steps=50, guidance_scale=7.5).images[0]
...
def initialize_pipeline(lora_weights_path):
...
if uploaded_file is not None:
    ...
    images = []
    st.write("Initializing image-generation model...")
    pipe = initialize_pipeline(lora_weights_path=lora_weights_path)
    for i, prompt in enumerate(prompts, start=1):
